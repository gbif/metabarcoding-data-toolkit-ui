...
const Uploader = ({onSuccess, onError, datasetKey, dataset}) => {
    ...
    const props = {
        accept: ".xlsx,.csv,.tsv,.txt,.zip,.qza,.biom,.h5,.hdf5,.he5,.fasta,.fa,.fas",
        multiple: true,
        onRemove: (file) => {
    ...
    return (
        <div style={{minHeight: "400px"}}>
            ...
            <Input  placeholder="Dataset title or nickname" value={datasetTitle} onChange={(e) => setDatasetTitle(e?.target?.value) } allowClear/> 
                <Text type="secondary" style={{marginBottom: "10px"}}>You may add or change the title later in the "Metadata" form in step 5</Text>
                </div>
